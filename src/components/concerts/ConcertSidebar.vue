<template>
     ...
     <q-btn
        ...
        <div class="info-container">
          ...
          <span
            class="info-label"
            v-if="rightMenuConcert.startTime.includes('|')"
            >Doors | Show</span
          >
          <span
            class="info-label"
        ...
        <div class="button-container">
          ...
          <q-card
            v-if="!going"
            class="icon-container"
            @click="handleInteraction(rightMenuConcert, 'GOING')"
          >
        ...
        <h3
          ...
          "
        >
          Discover {{ rightMenuConcert.artistName }}
        </h3>
          ...
          >
            <template v-slot:header>
              <q-item-section avatar>
                <q-avatar>
                  <img
                    src="https://amplify-venu20-dev-131644-deployment.s3.amazonaws.com/icons/icons8-youtube-48.png"
                    alt="Youtube icon"
                  />
                </q-avatar>
              </q-item-section>
              <q-item-section>
                <q-item-label class="text-white text-subtitle2"
                  >Youtube</q-item-label
                >
                <q-item-label class="text-white" style="font-size: 0.8em"
                  >View live performances</q-item-label
                >
              </q-item-section>
            </template>

            <q-video dark :src="rightMenuConcert.youtube" :ratio="16 / 9" />
          </q-expansion-item>
          <q-expansion-item
            dark
            expand-separator
          ...
          >
            <template v-slot:header>
              <q-item-section avatar>
                <q-avatar>
                  <img
                    src="https://amplify-venu20-dev-131644-deployment.s3.amazonaws.com/icons/icons8-spotify-96.png"
                    alt="Spotify icon"
                  />
                </q-avatar>
              </q-item-section>
              <q-item-section>
                <q-item-label class="text-white text-subtitle2"
                  >Spotify</q-item-label
                >
                <q-item-label class="text-white" style="font-size: 0.8em"
                  >Sample top songs</q-item-label
                >
              </q-item-section>
            </template>
            <q-list
              ...
              bordered
              separator
              v-for="(topTrack, index) in topTracks"
              :key="index"
            >
              <q-item
                dark
  ...
  methods: {
    ...
    async checkInteractions() {
      const goingInteraction = this.interactions.find(
        (interaction) =>
          interaction.concertId === this.rightMenuConcert.id &&
          interaction.status === "GOING"
      );
      const interestedInteraction = this.interactions.find(
        (interaction) =>
          interaction.concertId === this.rightMenuConcert.id &&
    ...
    async handleInteraction(data, status) {
      ...
      if (!currentState) {
        await this.$store.dispatch("profile/saveConcertInteraction", {
          data,
          status,
        });
        ...
        await this.$store.dispatch("profile/removeConcertInteraction", {
          data,
          status,
        });
    ...
    notifyMessage(data, isInterested, removal = false) {
      ...
      this.$q.notify({
        message: `${eventName} ${action} to calendar as ${
        ...
        actions: [
          {
            ...
            handler: () =>
              this.$router.push({
                path: "/profile",
                query: { date: this.rightMenuConcert.date },
              }),
          },
          {
            label: "Dismiss",
  ...
  computed: {
    selectedConcertVenue() {
      return this.venues.find(
        (venue) => venue.id === this.rightMenuConcert.venueConcertsId
      );
    },
    going: function () {
